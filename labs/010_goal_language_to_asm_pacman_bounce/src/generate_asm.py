...
SPRITE_PTR_GHOST = SPRITE_PTR_E_OPEN + 8
...
def ghost_sprite_rows() -> list[str]:
    # 24x21 single-color hardware sprite.
    # Simple solid ghost silhouette: rounded head, body, and feet.
    return [
        "........................",
        "........................",
        "........########........",
...
def generate(rows: list[str]) -> str:
    ...
    ghost_start_x, ghost_start_y = find_char(rows, "G")
...
TUNNEL_LEFT_X = $00
TUNNEL_RIGHT_X = $1b
TUNNEL_LEFT_CONTINUE_X = $01
TUNNEL_RIGHT_CONTINUE_X = $1a
PACMAN_STATE_ALIVE = $00
...
SPRITE_DATA_REMAINDER = {byte(sprite_bytes - 0x200)}
...
main_loop:
    jsr delay_frame
    lda pacman_state
    cmp #PACMAN_STATE_EXPIRING
    bne main_loop_not_expiring
    jsr update_expiration_state
    jmp main_loop

main_loop_not_expiring:
    lda pacman_state
...
position_ghost_sprite:
    jsr set_ghost_sprite_from_current_cell
    jsr update_ghost_sprite_registers
    rts

set_ghost_sprite_from_current_cell:
    ldx ghost_cell_x
    lda sprite_x_lo_by_cell,x
    sta ghost_sprite_x_lo
    lda sprite_x_hi_by_cell,x
    sta ghost_sprite_x_hi
    ldx ghost_cell_y
...
set_ghost_target_sprite_from_cell:
    ldx ghost_target_cell_x
    lda sprite_x_lo_by_cell,x
    sta ghost_target_x_lo
    lda sprite_x_hi_by_cell,x
    sta ghost_target_x_hi
    ldx ghost_target_cell_y
    lda sprite_y_by_cell,x
    sta ghost_target_y
    rts

update_ghost_sprite_registers:
    lda ghost_sprite_x_lo
    sta $d002
    lda ghost_sprite_y
    sta $d003
    lda $d010
    and #$fd
    sta $d010
    lda ghost_sprite_x_hi
    beq ghost_sprite_hi_done
    lda $d010
    ora #$02
    sta $d010
...
reset_ghost_to_start:
    lda #GHOST_START_CELL_X
    sta ghost_cell_x
    sta ghost_target_cell_x
    lda #GHOST_START_CELL_Y
    sta ghost_cell_y
    sta ghost_target_cell_y
    lda #GHOST_START_DIRECTION
    sta ghost_direction
    lda #$00
    sta ghost_motion_counter
    jsr set_ghost_sprite_from_current_cell
    jsr set_ghost_target_sprite_from_cell
    jsr update_ghost_sprite_registers
    rts
...
update_ghost_movement:
    jsr ghost_at_target
    beq ghost_smooth_continue
    jsr commit_ghost_target_cell
    jsr choose_ghost_next_target
    rts
ghost_smooth_continue:
    inc ghost_motion_counter
    lda ghost_motion_counter
    cmp #GHOST_PIXEL_SPEED_TICKS
    bcs ghost_smooth_step
    rts
ghost_smooth_step:
    lda #$00
    sta ghost_motion_counter
    jsr move_ghost_sprite_toward_target
    jsr update_ghost_sprite_registers
    rts

ghost_at_target:
    lda ghost_sprite_x_lo
    cmp ghost_target_x_lo
    bne ghost_not_at_target
    lda ghost_sprite_x_hi
    cmp ghost_target_x_hi
    bne ghost_not_at_target
    lda ghost_sprite_y
    cmp ghost_target_y
    bne ghost_not_at_target
...
commit_ghost_target_cell:
    lda ghost_target_cell_x
    sta ghost_cell_x
    lda ghost_target_cell_y
    sta ghost_cell_y
    rts

choose_ghost_next_target:
    jsr try_ghost_current_direction
    bcc ghost_target_chosen
    jsr choose_ghost_fallback_direction
ghost_target_chosen:
    jsr set_ghost_target_sprite_from_cell
    rts
...
try_ghost_current_direction:
    lda ghost_direction
    cmp #MASK_LEFT
    bne ghost_current_not_left
    jmp try_ghost_move_left
ghost_current_not_left:
    cmp #MASK_RIGHT
    bne ghost_current_not_right
    jmp try_ghost_move_right
ghost_current_not_right:
    cmp #MASK_UP
    bne ghost_current_not_up
    jmp try_ghost_move_up
ghost_current_not_up:
    cmp #MASK_DOWN
    bne ghost_current_not_down
    jmp try_ghost_move_down
ghost_current_not_down:
    sec
    rts
...
choose_ghost_fallback_direction:
    lda ghost_direction
    cmp #MASK_LEFT
    bne ghost_fallback_not_left
    jmp choose_ghost_fallback_from_left
ghost_fallback_not_left:
    cmp #MASK_RIGHT
    bne ghost_fallback_not_right
    jmp choose_ghost_fallback_from_right
ghost_fallback_not_right:
    cmp #MASK_UP
    bne ghost_fallback_not_up
    jmp choose_ghost_fallback_from_up
ghost_fallback_not_up:
    cmp #MASK_DOWN
    bne ghost_fallback_unknown
    jmp choose_ghost_fallback_from_down
ghost_fallback_unknown:
    jmp choose_ghost_fallback_from_left

choose_ghost_fallback_from_left:
    jsr try_ghost_move_up
    bcc ghost_fallback_done
    jsr try_ghost_move_down
    bcc ghost_fallback_done
    jsr try_ghost_move_right
    jmp ghost_fallback_done
...
try_ghost_move_left:
    jsr load_ghost_legal_mask
    and #MASK_LEFT
    beq ghost_move_left_blocked
    lda ghost_cell_x
    sec
    sbc #$01
    sta ghost_target_cell_x
...
move_ghost_sprite_toward_target:
    lda ghost_sprite_x_hi
    cmp ghost_target_x_hi
    bcc ghost_move_x_inc
    bne ghost_move_x_dec
    lda ghost_sprite_x_lo
    cmp ghost_target_x_lo
    bcc ghost_move_x_inc
    bne ghost_move_x_dec
    jmp ghost_move_y_check

ghost_move_x_inc:
    inc ghost_sprite_x_lo
    bne ghost_move_done
    inc ghost_sprite_x_hi
    jmp ghost_move_done

ghost_move_x_dec:
    lda ghost_sprite_x_lo
    bne ghost_move_x_dec_lo
    dec ghost_sprite_x_hi
ghost_move_x_dec_lo:
    dec ghost_sprite_x_lo
    jmp ghost_move_done

ghost_move_y_check:
    lda ghost_sprite_y
    cmp ghost_target_y
    bcc ghost_move_y_inc
    bne ghost_move_y_dec
    jmp ghost_move_done
...
check_ghost_collision:
    lda pac_cell_x
    cmp ghost_cell_x
    bne check_ghost_target_collision
    lda pac_cell_y
    cmp ghost_cell_y
    bne check_ghost_target_collision
    jsr trigger_pacman_expiration
    rts
check_ghost_target_collision:
    lda pac_cell_x
    cmp ghost_target_cell_x
    bne no_ghost_collision
    lda pac_cell_y
    cmp ghost_target_cell_y
    bne no_ghost_collision
    jsr trigger_pacman_expiration
no_ghost_collision:
    rts
...
trigger_pacman_expiration:
    lda #PACMAN_STATE_EXPIRING
    sta pacman_state
    lda #EXPIRATION_TICKS
    sta expiration_counter
    lda #SPRITE_PTR_E_CLOSED
    sta target_dir_ptr
    jsr set_sprite_pointer_current
    jsr update_sprite_registers
    rts
...
update_expiration_state:
    lda expiration_counter
    beq reset_after_expiration
    dec expiration_counter
    lda expiration_counter
    and #$08
    beq expiration_show
    lda #$02
...
choose_next_target_from_buffer:
    ...
    bne requested_not_left
    jmp try_requested_left
requested_not_left:
    cmp #SPRITE_PTR_E_OPEN
    bne requested_not_right
    jmp try_requested_right
requested_not_right:
    cmp #SPRITE_PTR_N_OPEN
    bne requested_not_up
    jmp try_requested_up
requested_not_up:
    cmp #SPRITE_PTR_S_OPEN
    bne requested_not_down
    jmp try_requested_down
requested_not_down:
...
try_tunnel_right:
    ...
    sec
    rts
tunnel_no_wrap_right:
    clc
    rts

try_requested_left:
    jsr try_tunnel_left
    bcc try_requested_left_no_tunnel
    rts
try_requested_left_no_tunnel:
    jsr load_legal_mask
    and #MASK_LEFT
...
try_requested_right:
    jsr try_tunnel_right
    bcc try_requested_right_no_tunnel
    rts
try_requested_right_no_tunnel:
    jsr load_legal_mask
    and #MASK_RIGHT
    beq continue_current_direction
    lda pac_cell_x
...
continue_current_direction:
    lda target_dir_ptr
    cmp #SPRITE_PTR_W_OPEN
    bne current_not_left
    jmp try_current_left
current_not_left:
    cmp #SPRITE_PTR_E_OPEN
    bne current_not_right
    jmp try_current_right
current_not_right:
    cmp #SPRITE_PTR_N_OPEN
    bne current_not_up
    jmp try_current_up
current_not_up:
    cmp #SPRITE_PTR_S_OPEN
    bne current_not_down
    jmp try_current_down
current_not_down:
    rts

try_current_left:
    jsr try_tunnel_left
    bcc try_current_left_no_tunnel
    rts
try_current_left_no_tunnel:
    jsr load_legal_mask
    and #MASK_LEFT
...
try_current_right:
    jsr try_tunnel_right
    bcc try_current_right_no_tunnel
    rts
try_current_right_no_tunnel:
    jsr load_legal_mask
    and #MASK_RIGHT
    beq stop_at_cell_center
    lda pac_cell_x
...
copy_sprite_page1:
    ...
    ldx #$00
copy_sprite_page2:
    lda custom_sprites+$0200,x
    sta SPRITE_DATA_ADDR+$0200,x
    inx
    cpx #SPRITE_DATA_REMAINDER
    bne copy_sprite_page2
    rts
...
ghost_cell_x:
    .byte GHOST_START_CELL_X

ghost_cell_y:
    .byte GHOST_START_CELL_Y
    ...
    intent = {
        ...
        "ghosts": {
            ...
            "stationary": False,
                ...
                "directionState": True,
                "spritePositionFromCurrentCell": True,
                "collisionUsesCurrentCell": True,
                "resetAfterPacmanExpiration": True,
                "legalMovement": True,
                "smoothMovement": True,
                "targetCellState": True,
                "spritePixelState": True,
                "commitCellAtTarget": True,
                "pixelInterpolation": True,
                "collisionChecksCurrentAndTargetCell": True,
                "movementUsesBoardLegalMasks": True,
                "currentDirectionFirst": True,
                "reverseOnlyAfterPerpendicularBlocked": True,
                "fallbackTurnPolicy": "try perpendicular turns before reversing",
                "pixelSpeedTicks": 4,
            "overlayDoesNotConsumeDots": True,
            "collisionTriggersExpiration": True,
            "movementDeferred": False,
                "speedDeferred": False,
                "animationDeferred": True,
                "eyesDeferred": True,
                "targetingDeferred": True,
                "multipleGhostsDeferred": True
        },
        "lives": {
            "enabled": True,
            "initialLives": 3,
            "expirationConsumesOneLife": True,
            "resetIfLivesRemain": True,
            "gameOverAtZero": True,
            "visibleDisplayDeferred": True
        },
        "pacmanExpiration": {
            "enabled": True,
            "stateTransition": "controlled expiration state, not immediate reset",
            "temporaryTrigger": "declared tunnel endpoints",
            "movementStopsDuringExpiration": True,
            "inputIgnoredDuringExpiration": True,
            "scoreRemainsUnchanged": True,
            "consumedItemsRemainConsumed": True,
            "resetAfterTimeout": True,
            "livesCounterDeferred": False
        },
        "tunnelWrap": {
            "enabled": True,
            "rule": "declared topology, not boundary escape",
            "row": 9,
            "leftEndpoint": {"x": 0, "y": 9},
            "rightEndpoint": {"x": 27, "y": 9},
            "leftFromLeftEndpointContinuesTo": {"x": 26, "y": 9},
            "rightFromRightEndpointContinuesTo": {"x": 1, "y": 9},
            "noGenericOffBoardMovement": True
        },
        "scoring": {
            "enabled": True,
            "scoreStartsAt": 0,
        ...
        "spriteProjection": {
            ...
            "spriteCopyFix": "copies all 576 bytes for nine sprite frames including the ghost body frame",
