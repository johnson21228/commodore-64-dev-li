...
OUT_PRG = LAB / "dist" / "p04_l_true_axis_endpoint_3x3_pit_preview.prg"
OUT_META = LAB / "dist" / "p04_l_true_axis_endpoint_3x3_pit_preview_metadata.json"
...
def load_base_projection():
    spec = importlib.util.spec_from_file_location("blockout_3x3_pit_projection", BASE_BUILDER)
...
def pit_dot_records() -> dict[int, int]:
    # 3x3x10 pit:
    # - cells are 3x3, so grid vertices are x/y 0..3.
    # - depth is 10, so z vertices are 0..10.
    # - draw many green reference points on side-wall/opening boundaries.
    # - no interior floor dots.
    base = load_base_projection()
    points: set[tuple[int, int]] = set()

    for z in range(0, 11):
        for x in range(0, 4):
            for y in range(0, 4):
                on_side_wall_or_opening = x in (0, 3) or y in (0, 3) or z == 0
                on_floor_interior = z == 10 and 0 < x < 3 and 0 < y < 3
                if on_side_wall_or_opening and not on_floor_interior:
                    points.add(base.project_vertex((x, y, z)))
...
def main():
    ...
    meta = {
        ...
        "program": str(OUT_PRG),
        "pieceId": "P04_L",
        "mode": "true-axis endpoint 3x3 pit preview",
        ...
        "runtimeTruth": "C64 runtime performs key-driven orientation table lookup and draws endpoint line commands against a 3x3x10 pit.",
        "pitStyle": "3x3x10 pit with single-pixel green side-wall/opening dots; no interior floor dots",
        "floorDots": False,
