...
PRG = LAB / "dist" / "p04_l_true_axis_endpoint_preview.prg"
META = LAB / "dist" / "p04_l_true_axis_endpoint_preview_metadata.json"
...
if not errors:
    ...
    if meta.get("mode") != "true-axis endpoint preview":
    ...
    if report.get("summary", {}).get("classification") != "PATCH":
        errors.append("source endpoint report should classify as PATCH")

if errors:
    print("ERROR: P04_L true-axis endpoint preview PRG verification failed.")
    for error in errors:
        print(f"- {error}")
    raise SystemExit(1)

print("OK: P04_L true-axis endpoint preview PRG verified.")
