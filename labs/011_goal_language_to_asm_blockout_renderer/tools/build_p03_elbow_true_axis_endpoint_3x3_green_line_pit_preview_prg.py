...
OUT_PRG = LAB / "dist" / "p03_elbow_true_axis_endpoint_3x3_green_line_pit_preview.prg"
OUT_META = LAB / "dist" / "p03_elbow_true_axis_endpoint_3x3_green_line_pit_preview_metadata.json"
...
def pit_dot_records() -> dict[int, int]:
    # Green line pit for 3x3x10:
    # vertices x/y 0..3, z 0..10.
    # Draw side-wall/opening grid lines. Avoid interior floor grid lines.
    base = load_base_projection()
    segments: set[tuple[tuple[int, int], tuple[int, int]]] = set()

    def add(a, b):
        p1 = base.project_vertex(a)
        p2 = base.project_vertex(b)
        if p1 != p2:
    ...
    for (x1, y1), (x2, y2) in sorted(segments):
        for x, y in base.bresenham((x1, y1), (x2, y2)):
...
def main():
    ...
    meta = {
        ...
        "program": str(OUT_PRG),
        "pieceId": "P03_ELBOW",
        "mode": "P03_ELBOW true-axis endpoint 3x3 green-line pit preview",
        ...
        "runtimeTruth": "C64 runtime performs key-driven orientation table lookup and draws P03_ELBOW endpoint line commands over a green 3x3x10 pit.",
        "pitStyle": "3x3x10 green projected side-wall/opening grid lines; no interior floor grid",
        "floorDots": False,
        "floorLines": False,
