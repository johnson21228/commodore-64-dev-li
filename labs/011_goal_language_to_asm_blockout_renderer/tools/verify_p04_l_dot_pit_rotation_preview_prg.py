...
PRG = LAB / "dist" / "p04_l_dot_pit_rotation_preview.prg"
META = LAB / "dist" / "p04_l_dot_pit_rotation_preview_metadata.json"
...
if not errors:
    ...
    if "dots-only" not in meta.get("pitStyle",""): errors.append("pitStyle must be dots-only")
    if meta.get("dotPitByteRecords",0) <= 0: errors.append("dot pit must have byte records")
...
if errors:
    print("ERROR: P04_L dot-only pit preview verification failed.")
    for e in errors: print(f"- {e}")
    raise SystemExit(1)
print("OK: P04_L dot-only pit rotation preview PRG verified.")
