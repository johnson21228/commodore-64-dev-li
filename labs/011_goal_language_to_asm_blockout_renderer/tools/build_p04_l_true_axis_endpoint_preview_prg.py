...
OUT_PRG = LAB / "dist" / "p04_l_true_axis_endpoint_preview.prg"
OUT_META = LAB / "dist" / "p04_l_true_axis_endpoint_preview_metadata.json"
...
def load_report():
    ...
    if report.get("pieceId") != "P04_L":
        raise RuntimeError("endpoint report must be for P04_L")
...
def pit_dot_records() -> dict[int, int]:
    # Reconstruct sparse wall/opening dots from the report's projected coordinates is not
    # necessary here; use the same single-pixel side-wall/opening dots from the prior preview.
    # The exact projection function is not needed because endpoint report already contains
    # active-block line coordinates, but the pit is easier as a small fixed byte-mask record
    # copied from projected boundary cells. Generate by reading the previous metadata/report
    # builder is overkill; keep it sparse and stable.
    #
    # Coordinates are chosen from the current lab projection neighborhood. They are only
    # reference points; floor dots are deliberately omitted.
    points = [
        (68,34),(98,34),(128,34),(158,34),(188,34),(218,34),
        (61,50),(91,50),(121,50),(151,50),(181,50),(211,50),
        (54,66),(84,66),(114,66),(144,66),(174,66),(204,66),
        (47,82),(77,82),(107,82),(137,82),(167,82),(197,82),
        (40,98),(70,98),(100,98),(130,98),(160,98),(190,98),
        (33,114),(63,114),(93,114),(123,114),(153,114),(183,114),
        (68,34),(61,50),(54,66),(47,82),(40,98),(33,114),
        (218,34),(211,50),(204,66),(197,82),(190,98),(183,114),
        (68,34),(78,48),(88,62),(98,76),(108,90),(118,104),
        (218,34),(228,48),(238,62),(248,76),(258,90),(268,104),
    ]
...
def build_code(symbols: dict[str, int], orientation_count: int, pit_record_count: int) -> bytes:
    ...
    jsr("load_orientation_payload")
    lda_imm(1); sta_zp(PLOT_WHITE)
    jsr("draw_endpoint_lines")
    a.emit(0x60)
...
def main():
    ...
    meta = {
        ...
        "program": str(OUT_PRG),
        "pieceId": "P04_L",
        "mode": "true-axis endpoint preview",
        ...
        "runtimeTruth": "C64 runtime performs key-driven orientation table lookup and draws endpoint line commands.",
        ...
        "knownArtifact": "Any green pit dot in an active-line 8x8 cell becomes white; sparse wall dots reduce collisions.",
        "sizes": sizes,
        "prgBytes": len(image),
